...
1. **Decouple Speed Prediction ($\mu$) from Uncertainty ($\sigma^2$):**  
   Never evaluate model quality solely via Gaussian Negative Log-Likelihood (NLL). An unconstrained neural network can deceptively minimize NLL by collapsing $\mu$ to the prior dataset mean ($\approx 32.6\text{ km/h}$) and inflating its predicted variance $\sigma^2$. Always mandate $\mu$-only RMSE and binned speed bias evaluations.
...
3. **Enforce Right-Hand Mathematical Coordinate Conventions:**  
   In Android `SensorManager`, $+Z$ gyroscope rate ($\omega_z$) is **counter-clockwise (CCW)**. In Math ENU coordinates ($x=\text{East}, y=\text{North}$), heading angle $\theta$ is measured CCW from East ($\dot{\theta} = \omega_z$). Geodetic compass heading is measured clockwise from North ($\psi = 90^\circ - \theta$). Inverting this convention reverses vehicle turns and causes catastrophic spatial divergence.
...
|---|---|---|---|
| **Phase A** | `SensorService` & `Alignment` | Raw hardware streaming + static gravity & dynamic GPS course alignment | Gravity error $< 0.1\text{ m/s}^2$; yaw offset convergence $< 2.0^\circ$ |
| **Phase B** | `StrapdownIns` & `EkfFusion` | 15-State ES-EKF with Non-Holonomic Constraints (NHC) | $0\text{ NaNs}, 0\text{ Divergences}$; covariance positive semi-definite |
| **Phase C** | `ModeManager` & GNSS Fusion | 1 Hz GNSS fusion + smooth blackout transition | GNSS-aided mean error $< 6.0\text{ m}$; final drift $< 0.1\%$ of total trip |
| **Phase D** | `SpeedFilterRunner` (AI) | 16-Channel Spectral Model (FFT PSD + physics) | $\text{MAE} < 8.0\text{ km/h}$ across all bins; 90s blackout drift $< 3.0\%$ |
| **Phase E** | `OsmMapMatcher` (HMM) | Offline OSM road-network topology snapping | Snapping latency $< 5\text{ ms}$; multi-minute blackout drift $< 2.0\%$ |
| **Phase F** | `DebugDashboard` (UI) | Real-time 3D orientation gizmo + 2D trail rendering | Sustained $10\text{ Hz}$ update rate; per-cycle latency $< 1.0\text{ ms}$ |
...
### Purpose:
Ensures the neural network dynamically tracks the true speed trajectory across all velocity regimes ($0\text{–}120\text{ km/h}$) and has not collapsed to the dataset prior mean ($\approx 32.6\text{ km/h}$).
...
=====================================================================================
```
* **Failure Condition:** If `Mean Pred` stays clamped near $30\text{–}35\text{ km/h}$ for the $\ge 70\text{ km/h}$ bins, the model is suffering from prior-mean collapse due to training set speed imbalance.
...
2. **OOD Sensitivity:** Out-of-distribution (OOD) or high-speed samples must have higher predicted $\sigma$ ($\ge 15\text{ km/h}$) than in-distribution urban samples ($\le 9\text{ km/h}$).
3. **Scatter Plot Verification:** Inspect [`ml/evaluation_plots/uncertainty_calibration_scatter.png`](file:///Users/anv./Development/INSS%20Navigation%20app/ml/evaluation_plots/uncertainty_calibration_scatter.png). Samples must fall below the $2\sigma$ confidence boundary ($|\text{Error}| \le 2\sigma$).
...
### Purpose:
Tests the entire closed-loop positioning engine (INS Mechanization $\to$ NHC Constraints $\to$ EKF Fusion $\to$ AI Speed Updates) against high-precision vehicle ECU ground truth over $5+\text{ km}$ drives with complex maneuvers (roundabouts, sharp turns, speed transitions) and a **90-second simulated GNSS blackout**.
...
* **Config (d): 90-Second Simulated Tunnel Outage** ($t = 120\text{s} \to 210\text{s}$).
...
1. **Symptom: Trajectory curves in opposite direction of road during maneuvers.**
   ...
   * **Fix:** In `StrapdownIns`, ensure $\dot{\theta} = \omega_z$ (where $\theta$ is Math ENU angle measured CCW from East). Verify compass heading conversion $\psi = 90^\circ - \theta$.
2. **Symptom: Error plot exhibits periodic $100\text{–}600\text{m}$ spikes during GNSS availability.**
   * **Root Cause:** Calculating GPS velocity by dividing static repeated coordinates by $\Delta t = 0.1\text{s}$, producing artificial $10\times$ velocity spikes on the 10th tick.
   * **Fix:** Only process GNSS position/velocity updates when a fresh, non-duplicate coordinate timestamp arrives.
3. **Symptom: Predicted speed remains $10\text{–}15\text{ km/h}$ when the car is stopped.**
   * **Root Cause:** Vehicle engine idle rumble ($800\text{ RPM} \approx 13.3\text{ Hz}$) shaking dashboard phone cradle.
   * **Fix:** Activate the physical Zero-Velocity Update (ZUPT) detector in `SpeedFilterRunner`. Clamps velocity to $0.0\text{ m/s}$ when $\text{Var}(a) < \text{threshold}$.
4. **Symptom: Predicted speed underpredicts highway cruising ($80\text{–}110\text{ km/h}$) by $>30\%$.**
   * **Root Cause:** Training dataset speed imbalance ($>81\%$ urban data $<50\text{ km/h}$).
...
5. **Symptom: EKF produces NaNs or negative diagonal covariance values.**
   * **Root Cause:** Numerical round-off in covariance update $\mathbf{P} = (\mathbf{I} - \mathbf{K}\mathbf{H})\mathbf{P}$.
   * **Fix:** Enforce symmetric Joseph form covariance update:
     ...
     and enforce positive semi-definite diagonal floor $\mathbf{P}_{ii} \ge 10^{-6}$.
