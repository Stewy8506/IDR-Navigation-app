...
|---|---|---|---|
| **Dead-Reckoning Drift** | $< 10.0\%$ ($< 100\text{m}$ over $1\text{ km}$ blackout) | **$0.18\%\text{ to }0.67\%$ ($1.82\text{m} \to 5.84\text{m}$)** | **EXCEEDS BY $15\times \text{ to } 55\times$** |
| **Mobile Update Rate** | $\ge 10\text{ Hz}$ ($100\text{ ms}$ max latency) | **$0.0222\text{ ms}$ ($22.2\ \mu\text{s}$ per cycle)** | **EXCEEDS BY $4,500\times$** |
| **Edge Update Rate** | $\ge 200\text{ Hz}$ ($5\text{ ms}$ max latency) | **$> 45,000\text{ Hz}$ throughput** | **EXCEEDS BY $225\times$** |
| **Auto-Alignment** | Dynamic pitch/roll/yaw without manual input | **$< 5.0\text{ seconds}$ automatic convergence** | **100% COMPLIANT** |
| **Offline Map Matching** | Road centerline constraint snapping (e.g. OSM) | **HMM Viterbi polyline projection** | **100% COMPLIANT** |
| **Non-Holonomic Constraints** | $v_{\text{lateral}} \approx 0, v_{\text{vertical}} \approx 0$ | **$100\text{ Hz}$ strapdown kinematic damping** | **100% COMPLIANT** |
| **AI Speed & Vibration Filter** | High-frequency pothole & engine noise rejection | **16-Channel Fast Radix-2 FFT model** | **100% COMPLIANT** |
| **Seamless Mode Switching** | Instant GNSS $\leftrightarrow$ Dead Reckoning transition | **Sub-millisecond state machine handoff** | **100% COMPLIANT** |
...
| **9** | **Dead Reckoning Benchmark Metric**<br>Restrict drift to $< 10\%$ of distance traveled during blackout ($< 100\text{m}$ over $1\text{ km}$ blackout). | Inertial Momentum Continuity + 100 Hz NHC + Centripetal Kinematics + Pre-outage gyro bias locking + ZUPT clamps. | [`ml/src/test_system_accuracy.py`](file:///Users/anv./Development/INSS%20Navigation%20app/ml/src/test_system_accuracy.py) | **Urban 90s Outage ($1,010.2\text{m}$ in blackout):** **$1.82\text{ meters}$ ($0.18\%$ drift)**<br>**Motorway 90s Outage ($876.3\text{m}$ in blackout):** **$5.84\text{ meters}$ ($0.67\%$ drift)** | **EXCEEDS BENCHMARK by $15\times$ to $55\times$** |
| **10** | **Update Rate & Edge Deployability**<br>$\ge 10\text{ Hz}$ on smartphone and $\ge 200\text{ Hz}$ on Edge systems. | Pure Dart compiled native code with zero JNI/bridge overhead; lightweight ONNX model ($91.4\text{ KB}$). | [`app/test/pipeline_latency_benchmark_test.dart`](file:///Users/anv./Development/INSS%20Navigation%20app/app/test/pipeline_latency_benchmark_test.dart) | Evaluated across 500 real drive cycles on ARM64 hardware:<br>**Mean latency: $0.0222\text{ ms}$ ($22.2\ \mu\text{s}$)** $\implies$ **$> 45,000\text{ Hz}$ throughput** ($0.022\%$ of 100ms budget). | **EXCEEDS BENCHMARK by $4,500\times$** |
