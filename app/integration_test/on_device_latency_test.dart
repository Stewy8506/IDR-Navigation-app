...
void main() {
  ...
  testWidgets('TASK 2: On-Device Physical ARM64 Latency Benchmark', (WidgetTester tester) async {
    // 1. Load real driving dataset asset
    final byteData = await rootBundle.load('assets/sample_logs/sample_drive.csv');
    final csvString = latin1.decode(byteData.buffer.asUint8List());
    final lines = const LineSplitter().convert(csvString);
    expect(lines.isNotEmpty, isTrue, reason: 'sample_drive.csv must load from asset bundle');
    ...
    // Warmup JIT / AOT compiler on device
    ...
    for (final line in lines) {
      ...
      // --- MEASURE 1: Feature Extraction & Alignment in Dart ---
      stopwatch.reset();
      stopwatch.start();
      ...
      // --- MEASURE 2: Strapdown INS Prediction & NHC in Dart ---
      stopwatch.reset();
      stopwatch.start();
      ...
      // --- MEASURE 3: EKF AI Speed Measurement Update in Dart ---
      stopwatch.reset();
      stopwatch.start();
      if (speedEst != null) {
      ...
      evaluatedSamples++;
      if (evaluatedSamples >= 1000) break;
    }
    ...
    print('\n' + '=' * 70);
    print('   TASK 2: ON-DEVICE PHYSICAL ANDROID ARM64 LATENCY BENCHMARK');
    print('=' * 70);
    print('Target Device:         Samsung Galaxy S24 FE (SM-S721B / ARM64)');
    print('Evaluated Cycles:      $evaluatedSamples consecutive real-data navigation ticks');
    print('-' * 70);
    print('Breakdown by Subsystem (Mean on ARM64):');
    ...
    print('-' * 70);
    print('Total Per-Cycle Dart Pipeline Latency (On Physical Device):');
