"""
standalone_exp6a_kaggle.py - Self-Contained Single-File Kaggle GPU Training Runner for Experiment 6A.
Can be pasted directly into a single Kaggle notebook code cell.

Contains:
  1. IMU Vehicle-Frame Alignment & Causal Physical Pitch Observer
  2. 18-Channel Feature Generator with Channel 15 Turn Gating
  3. ConvNeXt-1D + Temporal Multi-Head Attention + State Projection Network
  4. SequencePhysicsDataset (L=32, 10 Hz, zero GT leakage)
  5. Multi-Task Physics Losses (Huber + Calibrated NLL + ZUPT + Pitch + Regime + Delta_V)
  6. Closed-Loop Training Loop & Balanced 8-Bin Checkpoint Selection
"""
...
def auto_resolve_dataset_dir() -> str:
    ...
    for s_root in search_roots:
        ...
        if found:
            # Find the root folder containing the driver directories (e.g. S (Driver A))
...
def run_training():
    ...
    train_loader = DataLoader(train_ds, batch_size=64, shuffle=True, drop_last=True, num_workers=2 if device.type=="cuda" else 0, pin_memory=(device.type=="cuda"))
    ...
    # --- Pre-Flight Diagnostic Check: 1 Batch Forward & Backward Pass ---
    ...
    best_epoch = 0
    save_path_best = os.path.join(out_dir, "exp6a_best_spectral_speed_filter.pt")
    save_path_final = os.path.join(out_dir, "exp6a_final_spectral_speed_filter.pt")
    history = []

    print("Starting 15-Epoch Closed-Loop Training on GPU...\n", flush=True)
    ...
    for epoch in range(1, 16):
        ...
        # Validation (Driver A S3a continuous rollout)
        model.eval()
        val_preds, val_gts = [], []
        with torch.no_grad():
        ...
        print(f"Epoch [{epoch:02d}/15] ({ep_time:.1f}s) | Train Loss: {train_loss:.4f} | Train MAE: {train_mae:5.2f} km/h | Val MAE: {overall_val_mae:5.2f} km/h | Balanced: {balanced_val_mae:5.2f} km/h | r: {val_r:.3f}", flush=True)
        print(f"   Bin MAEs (km/h): [ {bin_report} ]", flush=True)

        history.append({"epoch": epoch, "train_loss": train_loss, "train_mae": train_mae, "val_mae": overall_val_mae, "balanced_val_mae": balanced_val_mae, "val_r": val_r, "bin_maes": bin_mae_dict, "epoch_time_s": ep_time})

        if balanced_val_mae < best_balanced_val_mae:
            best_balanced_val_mae = balanced_val_mae
            best_epoch = epoch
            torch.save(model.state_dict(), save_path_best)
            print(f"   >>> [SAVED BEST CHECKPOINT] Epoch {epoch} (Balanced MAE: {balanced_val_mae:.2f} km/h) <<<\n", flush=True)

    torch.save(model.state_dict(), save_path_final)
    with open(os.path.join(out_dir, "exp6a_history.json"), "w") as f:
        json.dump({"best_epoch": best_epoch, "best_balanced_val_mae": best_balanced_val_mae, "history": history}, f, indent=2)

    print(f"\nTraining Complete! Best Checkpoint saved to: {save_path_best}")
